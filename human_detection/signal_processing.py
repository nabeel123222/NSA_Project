...
def extract_features(amp_matrix, sample_rate_hz, apply_bandpass=True) -> CSIFeatures:
    ...
    if n_sub >= 2 and amp_matrix.shape[0] >= 3:
        ...
        selected = amp_matrix[:, sample_cols]

        # Remove constant subcarriers before correlation.
        # A constant column has zero standard deviation and causes
        # np.corrcoef() to generate divide-by-zero warnings.
        column_std = np.std(selected, axis=0)
        valid = column_std > 1e-12
